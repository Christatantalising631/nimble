...
impl Compiler {
    ...
    fn new_fn(name: &str) -> Self {
        Self {
            chunk: FunctionChunk::new(name.into()),
            next_reg: 0,
            locals: HashMap::new(),
            is_module: false,
            loops: Vec::new(),
    ...
    fn emit_jump_if_false(&mut self, cond: Reg) -> usize {
        self.chunk.emit(Instr::JumpIfFalse {
            cond,
            target: Addr(0),
        })
    }
    fn emit_jump_if_true(&mut self, cond: Reg) -> usize {
        self.chunk.emit(Instr::JumpIfTrue {
            cond,
            target: Addr(0),
        })
    }
        ...
        args.iter()
            .map(|arg| {
                ...
                let name = arg
                    .name
                    .as_ref()
                    .map(|name| self.chunk.add_name(name.clone()));
    ...
    fn compile_stmt(&mut self, stmt: &Stmt) {
        match &stmt.kind {
            StmtKind::Export(inner) => {
                let export_name = match &inner.kind {
                    StmtKind::FnDef(f) => Some(f.name.clone()),
                    StmtKind::Assign { target, .. } => Some(target.clone()),
                    StmtKind::ClsDef(c) => Some(c.name.clone()),
                    _ => None,
                };
                self.compile_stmt(inner);
                if let Some(n) = export_name {
                    self.chunk.exports.push(n);
                }
            }
            StmtKind::FnDef(fndef) => self.compile_fn_def(fndef),
            StmtKind::ClsDef(cls) => self.compile_cls_def(cls),
            ...
            StmtKind::While { cond, body } => {
                ...
                for p in &ctx.continue_patches {
                    self.patch_to(p, check_pc);
                }
                self.emit(Instr::Jump {
                    target: Addr(check_pc as u32),
                });
                self.patch_jump(jf);
            ...
            StmtKind::For {
                ...
                self.emit(Instr::ForIter {
                    dst: r_iter,
                    src: r_iter_val,
                    step: r_step,
                });
                let check_pc = self.pc();
                let done_patch = self.chunk.emit(Instr::IterNext {
                    var: r_var,
                    iter: r_iter,
                    done: Addr(0),
                });
                ...
                if let Some(prev_reg) = prev {
                    self.locals.insert(var.clone(), prev_reg);
                }
            }
            ...
            StmtKind::ForKV {
                ...
                if let Some(r) = prev_k {
                    self.locals.insert(key.clone(), r);
                }
                if let Some(r) = prev_v {
                    self.locals.insert(val.clone(), r);
                }
            }
            ...
            StmtKind::Continue => {
                if let Some((is_for, check_pc)) =
                    self.loops.last().map(|ctx| (ctx.is_for, ctx.check_pc))
                {
                    if is_for {
                        let p = self.emit_jump();
                        if let Some(ctx) = self.loops.last_mut() {
    ...
    fn compile_fn_def(&mut self, fndef: &FnDef) {
        ...
        if !sub.is_last_return() {
            sub.emit(Instr::Return { src: None });
        }
        sub.chunk.num_registers = sub.next_reg as u16;
    ...
    fn compile_cls_def(&mut self, cls: &ClsDef) {
        ...
        let cls_val = Value::Class {
            name: Arc::new(cls.name.clone()),
            fields,
        };
    ...
    fn compile_expr(&mut self, expr: &Expr) -> Reg {
        match &expr.kind {
            ...
            ExprKind::BinOp { op, left, right } => {
                ...
                let dst = self.alloc();
                let instr = match op {
                    BinOp::Plus => Instr::AddInt {
                        dst,
                        a: r_l,
                        b: r_r,
                    },
                    BinOp::Minus => Instr::SubInt {
                        dst,
                        a: r_l,
                        b: r_r,
                    },
                    BinOp::Star => Instr::MulInt {
                        dst,
                        a: r_l,
                        b: r_r,
                    },
                    BinOp::Slash => Instr::DivInt {
                        dst,
                        a: r_l,
                        b: r_r,
                    },
                    BinOp::Percent => Instr::Mod {
                        dst,
                        a: r_l,
                        b: r_r,
                    },
                    BinOp::EqEq => Instr::CmpEq {
                        dst,
                        a: r_l,
                        b: r_r,
                    },
                    BinOp::BangEq => Instr::CmpNe {
                        dst,
                        a: r_l,
                        b: r_r,
                    },
            ...
            ExprKind::UnaryOp { op, expr } => {
                ...
                match op {
                    UnaryOp::Minus => {
                        self.emit(Instr::Negate { dst, src: r });
                    }
                    UnaryOp::Not => {
                        self.emit(Instr::Not { dst, src: r });
                    }
                }
                dst
            }
            ...
            ExprKind::Map(pairs) => {
                let pair_regs: Vec<(Reg, Reg)> = pairs
                    .iter()
                    .map(|(k, v)| (self.compile_expr(k), self.compile_expr(v)))
                    .collect();
                let dst = self.alloc();
