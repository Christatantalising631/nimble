...
pub enum Instr {
    ...
    // ── Control flow ──────────────────────────────────────────────────────────
    Jump        { target: Addr },
    JumpIfFalse { cond: Reg, target: Addr },
    JumpIfTrue  { cond: Reg, target: Addr },
    Return      { src: Option<Reg> },

    // ── Calls and concurrency ─────────────────────────────────────────────────
    Call  { dst: Option<Reg>, callee: Reg, args: Vec<CallArgDesc> },
    Spawn { callee: Reg, args: Vec<CallArgDesc> },

    // ── Globals ───────────────────────────────────────────────────────────────
    LoadGlobal  { dst: Reg, name: NameIdx },
    StoreGlobal { name: NameIdx, src: Reg },

    // ── Collection constructors ───────────────────────────────────────────────
    MakeList   { dst: Reg, items: Vec<Reg> },
    MakeMap    { dst: Reg, pairs: Vec<(Reg, Reg)> },
    MakeRange  { dst: Reg, start: Reg, end: Reg },
    MakeStruct { dst: Reg, class: NameIdx, fields: Vec<(NameIdx, Reg)> },

    // ── Aggregate / string ops ────────────────────────────────────────────────
    Len       { dst: Reg, src: Reg },
    Concat    { dst: Reg, parts: Vec<Reg> },
    Stringify { dst: Reg, src: Reg },

    // ── Error handling ────────────────────────────────────────────────────────
    MakeError { dst: Reg, msg: Reg },
    Propagate { src: Reg },
    IsError   { dst: Reg, src: Reg },

    // ── Field / index access ──────────────────────────────────────────────────
    LoadField  { dst: Reg, obj: Reg, field: NameIdx },
    StoreField { obj: Reg, field: NameIdx, src: Reg },
    LoadIndex  { dst: Reg, obj: Reg, idx: Reg },
    StoreIndex { obj: Reg, idx: Reg, src: Reg },

    // ── Iterator support (for loops) ──────────────────────────────────────────
    /// Convert a list / range / string into an iterator value.
    ForIter  { dst: Reg, src: Reg, step: Option<Reg> },
    /// Advance the iterator in `iter`; place next element into `var`.
    /// Jumps to `done` when exhausted.
    IterNext { var: Reg, iter: Reg, done: Addr },
}
...
impl FunctionChunk {
    ...
    pub fn add_name(&mut self, name: String) -> NameIdx {
        for (i, n) in self.names.iter().enumerate() {
            if *n == name { return NameIdx(i as u16); }
        }
